...
export class AuthEffects {
  getAuthLogin = createEffect(() => this.actions$.pipe(
  ...
  getProfile = createEffect(() => this.actions$.pipe(
    ...
    rehydrateAuth = createEffect(() => this.actions$.pipe(
  ...
  persistAuth = createEffect(() => this.actions$.pipe(
  ...
  getAuthLogout = createEffect(() => this.actions$.pipe(
  ...
  forceLogout = createEffect(() => this.actions$.pipe(
      ofType(actions.FORCE_LOGOUT_401),
      map(action => new actions.RequestAuthLogoutAction())
    )
  );

  tabTokenChanges = createEffect(() => this.actions$.pipe(
  ...
  constructor(
    ...
    window.addEventListener('storage', event => {
      let newValue: any, key: string | null;
      ({ key, newValue } = event);
      try {
