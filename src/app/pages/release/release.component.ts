...
@Component({
  ...
  imports: [AsyncPipe, CommentListComponent],
...
export class ReleasePageComponent {
  ...
  constructor(
    ...
    this.route.params.subscribe((params) => {
      ...
      this.store.dispatch(
        new TrackActions.RequestForRelease({ releaseId: params.id }),
      );
      this.store.dispatch(
        new CommentActions.RequestForRelease({ releaseId: params.id }),
      );
      ...
      this.tracks$ = this.store.select(
        TrackSelectors.tracksForRelease(params.id),
      );
      this.comments$ = this.store.select(
        CommentSelectors.commentsForRelease(params.id),
      );
    });
  }
